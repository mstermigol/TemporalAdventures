...
<div class="container my-5">
    ...
    <form action="{{ route('admin.travel.update', $viewData['travel']->getId()) }}" method="POST"
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="title" name="title" value="{{ $viewData['travel']->getTitle() }}"
                required>
        </div>
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control custom-height-300" id="description" name="description" rows="3"
                required>{{ $viewData['travel']->getDescription() }}</textarea>
        </div>

        <div class="mb-3">
            <label for="place" class="form-label">@lang('admin.travel.place')</label>
            <input type="text" class="form-control" id="place" name="place" value="{{ $viewData['travel']->getPlace() }}"
                required>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control" id="price" name="price" value="{{ $viewData['travel']->getPrice() }}"
                required>
        </div>
